...
class UserReports:
    ...
    def load_all_users_list(self):
        ...
        self.logger.info("Loaded %s users.", str(len(all_users)))
        if len(all_users) != users_count:
            self.logger.warninging("Unable to load all users: %s", str(users_count))
    ...
    def report_users_recursive(self, dataverse_identifier):
        """Create users report recursively"""
    ...
    def load_users_recursive(self, users=None, dataverse_identifier=None):
        """Load Dataverse users recursively"""
        ...
        for dv_object in dataverse_contents:
            if dv_object['type'] == 'dataverse':
                ...
                self.load_users_recursive(users, dv_object['id'])
    ...
    def load_user_dataverse(self, users, dataverse_identifier):
        ...
        if 'data' in response_json:
            ...
            if 'dataverseContacts' in dataverse:
                ...
                self.logger.warning("Dataverse contact was empty.")
        else:
