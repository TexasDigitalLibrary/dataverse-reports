...
class DataverseApi(object):
		...
		def search(self, term='*', type='dataverse', options={}):
				if type is not None:
						url = self.host + 'api/search?q=' + term + '&type=' + type
				else:
						url = self.host + 'api/search?q=' + term
		...
		def get_dataverse(self, identifier=''):
				...
				url = self.host + 'api/dataverses/' + str(identifier)
		...
		def get_dataverse_contents(self, identifier=''):
				...
				url = self.host + 'api/dataverses/' + str(identifier) + '/contents'
		...
		def sword_get_dataverse(self, alias=''):
				...
				url = self.host + '/dvn/api/data-deposit/v1.1/swordv2/collection/dataverse/' + alias
		...
		def get_dataset(self, identifier=''):
				...
				url = self.host + 'api/datasets/' + str(identifier)
